...
class Legs3 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        videoView.setVideoURI(Uri.parse("android.resource://" + packageName + "/" + R.raw.calfraises))

        mediaPlayer = MediaPlayer.create(this, R.raw.calfraises)
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener { menuItem ->
            when (menuItem.itemId) {
                R.id.Calculate -> {
                    val calculateIntent = Intent(this@Legs3, Calculator::class.java)
                ...
                R.id.Home -> {
                    val calculateIntent = Intent(this@Legs3, Homepage::class.java)
                    calculateIntent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                    startActivity(calculateIntent)
                    overridePendingTransition(0, 0)
                    true
                ...
                R.id.Workout1 -> {
                    val calculateIntent = Intent(this@Legs3, Workout::class.java)
                    calculateIntent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                    startActivity(calculateIntent)
                    overridePendingTransition(0, 0)
                    true
                ...
                R.id.Setting -> {
                    val calculateIntent = Intent(this@Legs3, Profile::class.java)
                    calculateIntent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                    startActivity(calculateIntent)
                    overridePendingTransition(0, 0)
                    true
                ...
                R.id.Blog1 -> {
                    val calculateIntent = Intent(this@Legs3, Blog::class.java)
                    calculateIntent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                    startActivity(calculateIntent)
                    overridePendingTransition(0, 0)
                    true
        ...
        navigationView.setNavigationItemSelectedListener { menuItem ->
            when (menuItem.itemId) {
                R.id.menu_squats -> {
                    ...
                    startActivity(Intent(this@Legs3, Legs::class.java))
                    true
                }

                R.id.menu_lunges -> {
                    // Handle click for Overhead Press
                    startActivity(Intent(this@Legs3, Legs2::class.java))
                    true
                }
